...
void findReadColor(cv::Mat &hsv, cv::Mat &mask){
    cv::inRange(hsv, cv::Scalar(60, 105, 83), cv::Scalar(255, 255, 255), mask);
...
std::string output(int testid){
  ...
  if(testid==0)      return "re phai";
  else if(testid==1) return "re trai.";
  else if(testid==2) return "stop.";
  else return "casdfasdfasdfasdf";
